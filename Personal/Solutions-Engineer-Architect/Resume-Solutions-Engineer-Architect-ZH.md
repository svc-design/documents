...
| **AI 基础设施** | GPU 集群管理 · vLLM / SGLang / Ollama 模型服务 · MCP Server 集成 |
...
- 集成 Elasticsearch + Prometheus 监控体系，减少停机时间 30%。
...
**高级解决方案工程师 / 基础架构架构师** | *2024.11 – 2025.09*
**技术栈：** Linux · Kubernetes · DeepFlow · eBPF · GPU 基础设施 · 混合云 · AI Agents

- 为金融与电信企业客户设计并维护可观测性与 AIOps 平台。
- 优化 DeepFlow Collector 与 DF Server，支持高并发流量分析。
- 交付包括 **COSCO AIAgent 平台** 在内的 AI 驱动分析 PoC。
- 编写运行手册与自动化脚本，规范交付与维护流程。

**主要成果：**
- 完成 DFServer 性能优化与信创平台迁移。
- 解决电信金融类客户高并发丢包问题，系统稳定性提升 40%。
- 交付企业级 AI Agent + 可观测性一体化解决方案。

---

### 特斯拉（上海）有限公司
**站点可靠性工程师 / 平台工程师** | *2024.01 – 2024.05*
**技术栈：** Linux · Kubernetes · GitHub Actions · ArgoCD · Ansible · Helm · Jenkins · GPU 集群

- 维护支持工业控制与 AI 推理的混合 Kubernetes 集群。
- 使用 GitHub Actions 与 ArgoCD 实现自动化发布与回滚。
- 通过 Ansible 统一 Grafana 仪表盘与告警配置。
- 与内部 AI 团队合作，集成基于 LLM 的故障分析与事件关联。

**主要成果：**
- 通过 GitOps 自动化将部署周期缩短 40%。
- 构建安全的 GPU 混合云访问体系，实现云端与本地推理协同。
...
- 为企业级 MSP 客户设计并实施混合云自动化与可观测体系。
- 构建多区域 IaC 模块，实现跨云可复现部署。
- 引入基于 RAG 的 AIOps 流水线，实现事件关联与知识检索。
...
- 负责金融与互联网客户的混合云迁移方案设计与落地。
- 提供分布式存储、安全与可观测架构设计与支持。
- 编写 K8s 现代化与平台弹性架构设计文档。

**主要成果：**
- 为核桃教育交付多集群可观测平台（500+ 节点）。
- 完成 300+ 台主机与 多套 K8s 集群从腾讯云迁移至 UCloud。
- 撰写内部《容器平台迁移交付方案》，成为公司容器领域标准交付指南。
...
- 参与 **Deepin Server 企业版** 开发与自动化发布。
- 维护大规模 Linux 系统、CDN 与安全基础设施。
- 将 Linux 内核移植至 **龙芯（Loongson）MIPS 架构**，并实现基于 Koji 的 RPM 自动构建。
- 为金融与电信客户交付容器化与 DevOps 平台升级项目。
